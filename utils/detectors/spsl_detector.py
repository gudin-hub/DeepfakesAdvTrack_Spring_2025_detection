...
class SpslDetector(AbstractDetector):
    ...
    def features(self, input_tensor, phase_fea) -> torch.tensor:
        features = torch.cat((input_tensor, phase_fea), dim=1)
        return self.backbone.features(features)

    def classifier(self, features: torch.tensor) -> torch.tensor:
        return self.backbone.classifier(features)

    def forward(self, input_tensor, inference=False) -> dict:
        # get the phase features
        phase_fea = self.phase_without_amplitude(input_tensor)
        # bp
        features = self.features(input_tensor, phase_fea)
